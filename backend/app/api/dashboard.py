...
def portfolio_summary(
    ...
    for security_id, holding in holdings.items():
        ...
        if latest_dividend is not None:
            frequency = (holding["frequency"] or "quarterly").lower()
    ...
    watchlist_count = (
        ...
        .filter(AccountPosition.is_watchlist.is_(True))
        .count()
    )
    ...
    return PortfolioSummaryResponse(
        ...
        total_unrealized_gain_loss=quantize_money(
            total_unrealized_gain_loss
        ),
        monthly_dividend_income=quantize_money(
            monthly_dividend_income
        ),
        quarterly_dividend_income=quantize_money(
            quarterly_dividend_income
        ),
        annual_dividend_income=quantize_money(
